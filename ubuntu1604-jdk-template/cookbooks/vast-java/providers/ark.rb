...
# Co-Author:: Milos Buncic (<mbuncic@vast.com>)
# Cookbook Name:: vast-java
# Provider:: ark
#
# Copyright 2017, Vast.com
#
...
def parse_app_dir_name url
  ...
  if file_name =~ /^(jre|jdk).*$/
    ...
    # added support for jdk versions >= 9 (by <mbuncic@vast.com>)
    if major_num.to_i >= 9
      version = file_name.split('_')[0].split('-')[-1]
      app_dir_name = "#{package_name}-#{version}"
    else
...
def download_direct_from_oracle(tarball_name, new_resource)
  ...
  # jdk_id = case new_resource.url
  #          when /\/([678]u[0-9]+-b[0-9]+)\//
  #            new_resource.url.scan(/\/([6789]u[0-9]+-b[0-9]+)\//)[0][0]
  #          when /\/([9]\.[0-9]+\.[0-9]+\+[0-9]+)\//
  #            new_resource.url.scan(/\/([9]\.[0-9]+\.[0-9]+\+[0-9]+)\//)[0][0]
  #          end
  ...
  if node['java']['oracle']['accept_oracle_download_terms']
    ...
    converge_by(description) do
       ...
       cmd = shell_out!(
                                  ...
                                  %Q[ curl -L -H "Cookie: oraclelicense=accept" #{new_resource.url} -o #{download_path} ]
                               )
    end
  else
